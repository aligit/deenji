...
export class ReviewListComponent implements OnInit {
  ...
  private loadData() {
    ...
    // loadReviews returns a subscription
    this.reviewService.loadReviews(this.propertyId);

    // loadStats returns an observable, subscribe to it
    this.reviewService.loadStats(this.propertyId).subscribe({
      next: () => console.log('ReviewListComponent: stats loaded successfully'),
      error: (error) =>
        console.error('ReviewListComponent: error loading stats:', error),
    });
  }
